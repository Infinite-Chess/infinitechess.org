...
- The renderable factory is split across two files: the generic `createRenderableFactory` lives in [`webgl/Renderable.ts`](../src/client/scripts/esm/webgl/Renderable.ts) with the model types, while the game default and its free exports live in [`board/rendering/renderable.ts`](../src/client/scripts/esm/board/rendering/renderable.ts), because binding the default requires the board's camera and `webgl/` may not import from `board/`.
