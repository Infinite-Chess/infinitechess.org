...
How the redesign-era, per-component, TOML-based translation system works on the server and in templates. This is the dev reference — see [TRANSLATING.md](/docs/TRANSLATING.md) for translator workflow.
...
- **`i18next` dependency dropped.** The package is removed from `package.json`. Language resolution is handled by the `req.lang` lazy getter ([reqLanguage.ts](/src/server/config/reqLanguage.ts)).
...
- **[NAVIGATING.md](/docs/NAVIGATING.md) updated.** Its `src/client/views/` row still describes the legacy flow (EJS pages, the monolith `translation/en-US.toml`, and manual version bumps + `changes.json` entries). Rewrite it for the new system.
