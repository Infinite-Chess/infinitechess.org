...
- Sections in order: imports → Types → Elements → Constants → Schemas → State → functional
  groups → Exports. Every section gets a `// Section name -------` divider — never `=====`
  bars, never `// --- Name ---`. Pad the dashes so the line is exactly 80 characters.
