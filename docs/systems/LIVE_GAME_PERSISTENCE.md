...
| Column                       | Type    | Notes                                                                                                                                                                                                                                                                                                                    |
| ---------------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `log_time`                   | INTEGER | Epoch ms deadline to permanently log (lock in) a concluded game. NULL if ongoing. Set to `timeEnded + timeBeforeGameDeletionMillis` — a cushion for cheat reports to overturn the result first. On restoration, if elapsed, log immediately. (Once logged, the row is deleted, so a logged game is never in this table.) |
...
| **Draw accepted**                                           | `conclusion_condition`, `conclusion_victor`, `time_ended`, `draw_offer_state`, `log_time`                     | —                                                                                                   |
| **Resignation**                                             | `conclusion_condition`, `conclusion_victor`, `time_ended`, `log_time`                                         | —                                                                                                   |
| **Abort**                                                   | `conclusion_condition`, `time_ended`, `log_time`                                                              | —                                                                                                   |
| **Time loss**                                               | `conclusion_condition`, `conclusion_victor`, `time_ended`, `color_ticking`, `clock_snapshot_time`, `log_time` | `time_remaining_ms`                                                                                 |
| **Claim victory/draw**                                      | `conclusion_condition`, `conclusion_victor`, `time_ended`, `log_time`, `both_disconnected_end_time` → NULL    | —                                                                                                   |
...
| **Both-disconnected timeout** (draw by abandonment / abort) | `conclusion_condition`, `conclusion_victor`, `time_ended`, `log_time`, `both_disconnected_end_time` → NULL    | —                                                                                                   |
| **Game logged** (result locked in)                          | DELETE row (cascades to `live_player_games`) — result now lives in the permanent tables                       | (cascades)                                                                                          |
| **Game evicted** (both players left the rematch window)     | — (row already removed at logging)                                                                            | —                                                                                                   |

> **Post-game rematch window:** After a game concludes it is logged (immediately if server-validated, else after the `log_time` cheat-report cushion). Logging **deletes the row** — the result is now safe in the permanent tables — but the game **lingers in memory** to host the rematch handshake until both players leave, at which point it is evicted from memory. The rematch offers and post-game reconnection cushions are ephemeral (never persisted). A concluded-but-not-yet-logged game (still in its cheat-report cushion) is the only post-conclusion state that survives a restart: it restores and resumes its `log_time` deadline. A logged game is never in this table, so there's nothing to discard on restart.
