...
every user-facing email is sent in direct response to a request from a localized page. No locale
needs to be stored anywhere.

> **Prerequisite (not part of this task).** The email **visual redesign**
> (`1_email_redesign.md`) should already be done — we localize the redesigned HTML, not the old plain text. Localizing
...
| Password-changed receipt (`sendPasswordChangedEmail`) | the user | **Yes** |
...
These senders' callers already have the request language in hand (`req.lang`):
...
- `sendPasswordChangedEmail` — called from `passwordResetController.handleResetPassword`.

## Do
1. **Thread the language through.** Add a `language: string` parameter to `sendEmailConfirmation`,
   `sendPasswordResetEmail`, and `sendPasswordChangedEmail`, and pass `req.lang` from each caller.
   Leave `sendRatingAbuseEmail` untouched.
2. **Create a dedicated `email` translation component.** Add `translation/email/en-US.toml` with
   ...
   `[verify]` / `[reset]` / `[receipt]` sub-tables — subject, heading, body line(s), button label, and the
...
4. **Keep the shared shell language-agnostic.** The email-layout shell from the redesign task
   (`1_email_redesign.md`) takes a title
...
6. **Preview** the verification, password-reset, and password-changed-receipt emails in at least
   one non-English language and confirm the subject, body, button/link, and fallback are all
   translated and still render well.

## Out of scope
- The rating-abuse email (internal — stays English).
- The email visual design (`1_email_redesign.md`) and any link/token mechanics.
...
- Verification, password-reset, and password-changed-receipt emails render fully in the
  recipient's `req.lang` — subject, body, button/link label, and plain-text alternative all
  localized, with interpolated values (username / expiry) correct.
