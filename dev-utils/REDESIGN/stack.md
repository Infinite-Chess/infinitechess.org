...
- **`.njk` templates live in `src/server/views/`** and are copied to `dist/server/views/` by the cpx build step. Nunjucks is configured to read from `dist/server/views/` at runtime. In dev, cpx watches `src/server/views/` and mirrors edits to `dist/`, at which point Nunjucks's own file watcher picks them up — no server restart required.
...
- **CSS methodology:** One shared stylesheet (`src/client/css/global.css`) for global styles, plus a per-page stylesheet in `src/client/css/` for each page (e.g. `src/client/css/index.css`). Short, descriptive class names scoped with native CSS nesting — no BEM, no prefixes. Each page's stylesheet has one top-level block matching its `<main>` class (e.g. `.login { .form-field {} }`), preventing any bleed between pages. lightningcss in the existing build pipeline handles transpilation for older browsers. Utility classes (`.hidden`, `.italic`, `.flex`, etc.) are hand-rolled and added to the shared stylesheet when redundancy appears — no Tailwind. Component CSS is colocated with the component it styles (e.g. `src/client/components/header/header.css`).
...
- **Font stack:** `"Noto Sans", Verdana, sans-serif` (Lichess font). Self-host Noto Sans (not loaded from Google Fonts CDN, but via `@font-face`) to avoid the extra DNS/connection overhead and the 1-day CSS cache expiry that Google Fonts carries. Font files are served with `Cache-Control: max-age=31536000` alongside other static assets. To update a font, change the `@font-face` src URL and bump `?v=N` on any direct template references.
