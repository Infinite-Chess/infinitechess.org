...
- **`.njk` templates go through the cpx copy step, not esbuild.** The build pipeline handles them the same way as CSS and images — `cpx` mirrors `src/client/**/*.njk` into `dist/client/`, and Nunjucks reads them from `dist/client/` at runtime (the same root as all other static assets). This is a deliberate consistency: the server process runs out of `dist/`, so `__dirname` at runtime points into `dist/server/`, and the Nunjucks root is `dist/client/`. Template edits in `src/` propagate to `dist/` via cpx in dev watch mode, at which point Nunjucks's own file watcher picks them up — no server restart required.
