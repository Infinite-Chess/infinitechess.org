# Part 1 — Chat System — Requirements

Decided requirements for the game-page chat, split into behavior (what the user sees and
experiences) and the backend design.

**The report system is a separate system**, kept in its own part of this document — see
"Part 2 — Report System" below. It consumes the chat's data but shares none of its lifecycle,
wire or storage decisions. Nothing in Part 1 depends on it.
...
# Part 2 — Report System — Requirements
...
plus a log file), its own CSS classes, its own rate limiting. Part 1 has no dependency on
Part 2; the chat ships and works with the flag button inert.
...
identifiable after eviction. This mirrors what Part 1 already decided for the chat *input*.
...
**English** shared translations. (The email is to the owner; Part 1's standing rule of hardcoded
English until the page-wide localization pass applies here too.)
...
| 7 | **Where the server-side report module lives**, and its file name. | Part 1 puts chat at `src/server/game/gamemanager/chat.ts`; reporting is a separate system and need not sit there. |
...
| 10 | **Server-side failure handling** — what happens if the email send or log write throws. | Part 1's chat-write rule (swallow, skip the broadcast) is a precedent, but reporting has no broadcast. |

---

## Deferred — chat moderation (not part of the report system)
...
Whether this ships alongside the report system is undecided.
