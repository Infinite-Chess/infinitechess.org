...
- **Chat system.** Built and shipped — behavior, static event notices, backend design, and the report flag. Its two requirement docs were deleted once the code superseded them; read `chat.ts`, `chatentry.ts`, `guichat.ts` and `chatReport.ts`.
- **Game actions:**
