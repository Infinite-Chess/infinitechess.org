...
- Variant groups: Standard (no gamerule modifications, mixed rated + finite unrated), Horde (custom win conditions), 4 Dimensional (custom piece movements), Showcase (custom win conditions, black moves first, not intended to be competitive, mate in omega), Custom (choose between inputing custom icn, or selecting one of your saved editor positions).
- Choosing a variant has a nested popup of some kind, with all variants grouped into the above groups. Each group type has its own svg associated with that type, this is so that at a glance, open invites in the lobby show what group the game belongs to, conveying gamerule modifications at a glance. Hovering over variant names displays a new small canvas tooltip which renders a preview of the variant's starting position. After selecting a variant, an edit button is visible which opens the ICN text field below, pre-filled with the selected variant's ICN.
...
- FUTURE: A way to add special rule modifications to the selected variant: Fog of War, Duck Chess, Antichess, Atomic, KoTH, Slide Limit, Chess 960, Obstocean (infinite obstacles), Drawback Chess, Progressive Chess. None are mutually-exclusive, add as many as desired. Adding a modification shows that respective symbol in the Variant column of the seek.
