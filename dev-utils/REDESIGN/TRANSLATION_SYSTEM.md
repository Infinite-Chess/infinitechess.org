...
How the redesign-era, per-component, TOML-based translation system works on the server and in templates. This is the dev reference — see [TRANSLATIONS.md](./TRANSLATIONS.md) for translator workflow.
...
- **[src/client/types/client-translations.d.ts](../src/client/types/client-translations.d.ts)** — declares `const t: ClientTranslations`, one property per component that has a `[client]` table. Client scripts get autocompletion on `t.header.x.y`.
...
The system today runs **alongside** the legacy i18next system. The redesign roadmap ([dev-utils/REDESIGN/todo.md § Translation System Refactor](../dev-utils/REDESIGN/todo.md)) removes the old system entirely. Once that work lands, expect the following differences from what's documented above:
