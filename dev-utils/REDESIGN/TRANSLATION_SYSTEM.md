...
- **`i18next` dependency dropped.** The package is removed from `package.json`. Language resolution is handled by a custom Accept-Language parser middleware that replaces today's `getLanguageToServe()` in [translate.ts](../../src/server/utility/translate.ts). The middleware reads the same precedence (cookie → Accept-Language → default) but without the i18next runtime, resolving the language once per request so the request-bound translator (below) can reuse it. Expose the Accept-Language parsing as a standalone pure function (raw cookie value + `Accept-Language` header → lang), not just as middleware: the early rate-limit middleware in [rateLimit.ts](../../src/server/middleware/rateLimit.ts) runs before this middleware and resolves the language itself in `getRequestLanguage()`. Point `getRequestLanguage()` at that shared parser so it stops hand-rolling resolution, picks up the renamed cookie, and — by reading the raw `Accept-Language` header (available pre-middleware, unlike `req.i18n`) — finally localizes rate-limit responses for first-time visitors.
