...
- Output all games in the database in a json the same format the icnconverter accepts. Determine if there are any with illegal moves played (can only happen if the opponent failed to report). If there are any, decide whether to keep or delete them.
