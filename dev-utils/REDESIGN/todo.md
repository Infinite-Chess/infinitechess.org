...
- Write the GitHub Actions deploy workflow file with the three triggers. Verify each of them work as intended. Verify that restarts have near zero downtime (verified max 2.5 seconds).

- Websocket downtime on server restarts is only 2.5 seconds--quick enough that we don't need to compensate players' clocks. Remove `allowinvites.json` and its polling mechanism — live game persistence makes pre-restart blocking unnecessary. Remove that and any feature added that is related to it. ie clients no longer have to display how long until the server restarts. Some translation keys in en-US.toml then become unused, remove those and indicate the change in changes.json. Don't forget to update the websocket zod schemas so they don't allow the server restart in messages in websocket message bodies anymore.. and remove anything else related to the feature I may have forgotten. There is no more functionality for disabling invite creation (in anticipation of a server restart). allowinvites.json may be deleted and its managing script.
