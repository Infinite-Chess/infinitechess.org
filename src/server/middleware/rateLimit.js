...
function rateLimit(req, res, next) {
    ...
    if (!clientIP) {
        ...
        return res.status(500).json({ message: "unable_to_identify_client_ip" });
    }
    ...
    if (isIPBanned(clientIP)) {
        ...
        return res.status(403).json({ message: "you_are_banned_by_server" });
    }
    ...
    if (rateLimitHash[clientIP] > maxRequestsPerMinute) { // Rate limit them (too many requests sent)
        ...
        return res.status(429).json({ message: "too_many_requests_to_server" });
    }
