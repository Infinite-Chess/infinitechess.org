...
function injectScript(
  ...
  if (Object.keys(stringInjection).length != 0) {
    modifiedHTML = modifiedHTML.replace(
      stringInjection.injectafter,
      `${stringInjection.injectafter}${stringInjection.string}`,
    );
  }
  return modifiedHTML;
}
...
function injectHtmlscript() {
  ...
  const game_JSscripts = glob
    .sync(`./dist/scripts/game/**/*.js`)
    .filter((file) => {
      return !/htmlscript\.js/.test(file);
    });
  // Convert the list of scripts into an explicit HTML string that imports them all
  for (file of game_JSscripts) {
    const js_filename = file
      .split(/(\\|\/)+/)
      .slice(4)
      .join(""); // discard "dist/scripts/"
