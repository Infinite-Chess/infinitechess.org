...
async function migrateGameLogsToDatabase() {
	...
	rl.on('line', async(line) => {
		...
		// line starts with metadata
			...
			if (longformOut.metadata.White !== "(Guest)") game.players[1].identifier = { member: longformOut.metadata.White, user_id: uuid.base62ToBase10(longformOut.metadata.WhiteID!) };
			else game.players[1].identifier = { browser: 'examplebrowserid' };
			if (longformOut.metadata.Black !== "(Guest)") game.players[2].identifier = { member: longformOut.metadata.Black, user_id: uuid.base62ToBase10(longformOut.metadata.BlackID!) };
