 ...
 * @param {number} id - The id of the game they are in.
 */
 ...
 * @param {Object} user - An object containing either the `member` or `browser` property.
 * @param {number} gameID - The id of the game they are in.
 */
function removeUserFromActiveGame(user, gameID) { // { member/browser }
	...
	if (user.member) {
		...
		else if (membersInActiveGames[user.member] !== undefined) console.log("Not removing member from active games because they speedily joined a new game!");
	} else if (user.browser) {
		if (browsersInActiveGames[user.browser] === gameID) delete browsersInActiveGames[user.browser];
		else if (browsersInActiveGames[user.browser] !== undefined) console.log("Not removing browser from active games because they speedily joined a new game!");
...
function isSocketInAnActiveGame(ws) {
	...
	if (player.member) { // Their username trumps their browser id.
		if (membersInActiveGames[player.member] !== undefined) return true;
	...
	} else if (player.browser && browsersInActiveGames[player.browser] !== undefined) return true;
