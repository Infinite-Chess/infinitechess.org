// src/server/game/gamemanager/chatEntryMapper.ts
 ...
 */
function toEntry(data: ChatEntryData, index: number): ChatEntry {
 ...
 */
function toLogEntry(data: ChatEntryData, index: number, now: number): ChatLogEntry {
	return { ...toEntry(data, index), millisAgo: now - data.sent_at };
}

// Exports ---------------------------------------------------------------------

export default { toEntry, toLogEntry };
