...
function onReport(
	...
	if (servergame.boardsim !== undefined) {
		const errString = `Player tried to report cheating in a server-validated game (unnecessary). Report message: ${JSON.stringify(messageContents)}. Reporter color: ${ourColor}.\nThe game: ${gameutility.getSimplifiedGameString(servergame)}`;
		...
		gameutility.sendMessageToSocketOfColor(
			...
			'general',
			'printerror',
			'Cannot report opponent in this variant; the server validates all moves automatically.',
		);
		return;
	}
