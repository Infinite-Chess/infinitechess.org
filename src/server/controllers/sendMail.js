 ...
 */
function sendRatingAbuseEmail(messageText) {
	const EMAIL_USERNAME = process.env.EMAIL_USERNAME;
	const EMAIL_APP_PASSWORD = process.env.EMAIL_APP_PASSWORD; // App password generated by google, instead of using our main password

	// Check if the email environment variables exist
	if (EMAIL_USERNAME === "" || EMAIL_APP_PASSWORD === "") {
		console.log("Email environment variables not specified. Not sending email to admin account about rating abuse.");
		return;
	}
	...
	const mailOptions = {
		...
		subject: 'Rating Abuse Warning',
		text: messageText
	};

	transporter.sendMail(mailOptions, function(err, info) {
		if (err) logEventsAndPrint(`Error when sending rating abuse email: ${err.stack}`, 'errLog.txt');
		else console.log(`Rating abuse email notification sent successfully!`);
	});
}
