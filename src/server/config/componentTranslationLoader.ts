 ...
 * server: the TOML object with [client] removed — used by SSR templates.
 * client: the [client] sub-table with keys promoted one level up — served to browser JS.
 */
type ComponentEntry = { server: Record<string, any>; client: Record<string, any> };
...
/** The root folder containing per-component translation subdirectories. */
 ...
 */
function loadComponentTranslations(): void {
	...
	for (const componentName of componentDirs) {
		const componentDir = path.join(translationsFolder, componentName);
		const tomlFiles = fs.readdirSync(componentDir).filter((f) => f.endsWith('.toml'));

		// Skip component directories that have no English source file
		const englishTomlPath = path.join(componentDir, `${DEFAULT_LANGUAGE}.toml`);
		if (!fs.existsSync(englishTomlPath)) continue;

		const englishRaw = html_escape(parse(fs.readFileSync(englishTomlPath, 'utf-8')));
		const englishServerObj = withoutClientTable(englishRaw);
		const englishClientObj = (englishRaw['client'] ?? {}) as Record<string, any>;
		...
		for (const file of tomlFiles) {
			...
			const raw = html_escape(parse(fs.readFileSync(path.join(componentDir, file), 'utf-8')));
 ...
 * Throws if loadComponentTranslations() has not been called yet, or if the component doesn't exist.
 *
 * @param component - The component name, e.g. "header"
 * @param lang - The language code, e.g. "de-DE"
 */
function getComponentTranslation(component: string, lang: string): Record<string, any> {
