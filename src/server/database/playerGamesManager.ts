...
function getPlayerGamesData(
	...
	try {
		...
		if (!row) {
			logEventsAndPrint(
				`No matches found in player_games table for user_id = ${user_id} and game_id = ${game_id}.`,
				'errLog.txt',
			);
			return undefined;
		}
		...
		logEventsAndPrint(
			`Error executing query when gettings player game of user_id ${user_id} and game_id = ${game_id}: ${message}. The query: "${query}"`,
			'errLog.txt',
		);
		return undefined;
	}
...
function getPlayersInGame(game_id: number): PlayerGamesRecord[] {
	...
	try {
		...
		const message = error instanceof Error ? error.message : String(error);
		logEventsAndPrint(
			`Error getting all player_games entries for game "${game_id}": ${message}`,
			'errLog.txt',
		);
...
function updatePlayerGamesColumns(
	user_id: number,
	game_id: number,
	columnsAndValues: PlayerGamesRecord,
): ModifyQueryResult {
	...
	if (typeof columnsAndValues !== 'object' || Object.keys(columnsAndValues).length === 0) {
		logEventsAndPrint(
			`Invalid or empty columns and values provided for user ID "${user_id}" and game ID "${game_id}" when updating player_games columns! Received: ${jsutil.ensureJSONString(columnsAndValues)}`,
			'errLog.txt',
	...
	for (const column in columnsAndValues) {
		...
		if (!allPlayerGamesColumns.includes(column)) {
			logEventsAndPrint(
				`Invalid column "${column}" provided for user ID "${user_id}" and game ID "${game_id}" when updating player_games columns! Received: ${jsutil.ensureJSONString(columnsAndValues)}`,
				'errLog.txt',
			); // Detailed logging for debugging
	...
	try {
		...
		if (result.changes > 0) return { success: true, result };
			...
			logEventsAndPrint(
				`No changes made when updating player_games table columns ${JSON.stringify(columnsAndValues)} for game in player_games table with user ID "${user_id}" and game ID "${game_id}"! Received: ${jsutil.ensureJSONString(columnsAndValues)}`,
				'errLog.txt',
			);
		...
		logEventsAndPrint(
			`Error updating player_games table columns ${JSON.stringify(columnsAndValues)} for user ID "${user_id}" and game ID "${game_id}": ${message}! Received: ${jsutil.ensureJSONString(columnsAndValues)}`,
			'errLog.txt',
		);
