...
const MOVE_RULE_REGEX = new RegExp(
	String.raw`(?<moveRule>${icnposition.WHOLE_NUMBER_SOURCE}/${icnposition.COUNTING_NUMBER_SOURCE})${WHITE_SPACE_OR_END}`,
	'y',
);

const FULL_MOVE_REGEX = new RegExp(
	String.raw`(?<fullMove>${icnposition.COUNTING_NUMBER_SOURCE})${WHITE_SPACE_OR_END}`,
	'y',
);
...
const MOVES_REGEX = new RegExp(
	String.raw`(?<moves>${icnmoves.MOVES_REGEX_SOURCE})${WHITE_SPACE_OR_END}`,
	'y',
);
...
function LongToShort_Format(
	...
	if (longformat.gameRules.promotion) {
		...
		for (const player of uniquePlayers) {
			...
			playerSegments.push(ranksString);
	...
	if (longformat.moves && longformat.moves.length > 0) {
		...
		segments.push(icnmoves.getShortFormMovesFromMoves(longformat.moves, move_options));
	}
...
function ShortToLong_Format(icn: string): LongFormatOut {
	...
	if (promotionsMatch) {
		...
		for (const player of uniquePlayers) {
			...
			if (ranks) promotionRanks[player] = ranks.split(',').map((r) => BigInt(r));
			if (allowed) {
				// prettier-ignore
...
function matchMovesSection(
	...
	if (!match) return undefined;
	return {
		moves: icnmoves.parseShortFormMoves(match.groups['moves']!),
...
function parsePresetSquares(presetSquares: string): Coords[] {
	...
	const squares: Coords[] = coordsKeys.map((key) => coordutil.getCoordsFromKey(key));
