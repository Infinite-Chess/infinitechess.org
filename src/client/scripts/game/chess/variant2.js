...
const variant2 = (function() {
    ...
    const variantDictionary = {
        Classical: {
            ...
            gameruleModifications: { promotionsAllowed: defaultPromotions }
        },
        Core: {
        ...
        Standarch: {
            ...
            gameruleModifications: { promotionsAllowed: [...defaultPromotions,'chancellors','archbishops'] }
        },
        Space_Classic: {
            positionString: {
            ...
            gameruleModifications: {
                // UTC Feb 27, 2024, 7:00
                1709017200000: { promotionsAllowed: defaultPromotions }, // Use standard [8,1] promotion lines
                0: { promotionRanks: [4,-3], promotionsAllowed: defaultPromotions }
            }
        },
        CoaIP: {
            positionString: positionStringOfCoaIP,
            gameruleModifications: { promotionsAllowed: coaIPPromotions }
        },
        Pawn_Horde: {
            ...
            gameruleModifications: { winConditions: { white: ['checkmate'], black: ['allpiecescaptured'] }, promotionRanks: [2,-7], promotionsAllowed: defaultPromotions }
        },
        Space: {
            ...
            gameruleModifications: { promotionRanks: [4,-3], promotionsAllowed: [...defaultPromotions,'hawks','centaurs','archbishops','chancellors'] }
        },
        Obstocean: {
        ...
        Abundance: {
            ...
            gameruleModifications: { promotionRanks: [6,-6], promotionsAllowed: defaultPromotions }
        },
        Amazon_Chandelier: {
            ...
            gameruleModifications: { promotionRanks: [10,-9], promotionsAllowed: [...defaultPromotions,'hawks','guards','amazons'] }
        },
        Containment: {
        ...
        Classical_Limit_7: {
            positionString: positionStringOfClassical,
            gameruleModifications: { slideLimit: 7, promotionsAllowed: defaultPromotions }
        },
        CoaIP_Limit_7: {
            positionString: positionStringOfCoaIP,
            gameruleModifications: { slideLimit: 7, promotionsAllowed: coaIPPromotions }
        },
        Chess: {
        ...
        Classical_KOTH: {
            positionString: positionStringOfClassical,
            gameruleModifications: { winConditions: getWinConditionsOfKOTH(), promotionsAllowed: defaultPromotions }
        },
        CoaIP_KOTH: {
            positionString: positionStringOfCoaIP,
            gameruleModifications: { winConditions: getWinConditionsOfKOTH(), promotionsAllowed: coaIPPromotions }
        },
        Classical_Plus: {
        ...
        Knightline: {
            ...
            gameruleModifications: { promotionsAllowed: { white: ['knights','queens'], black: ['knights','queens'] } }
        },
        Knighted_Chess: {
            positionString: {
            ...
            gameruleModifications: { promotionsAllowed: [] }
        },
        Omega: {
            positionString: 'r-2,4|r2,4|r-2,2|r2,2|r-2,0|r0,0|r2,0|k0,-1|R1,-2|P-2,-3|Q-1,-3|P2,-3|K0,-4',
            gameruleModifications: { promotionRanks: null, moveRule: null, turnOrder: getTurnOrderOfOmega() }
        },
        Omega_Squared: {
        ...
        Omega_Cubed: {
            generator: {
                algorithm: variantomega.genPositionOfOmegaCubed,
        ...
        Omega_Fourth: {
            generator: {
                algorithm: variantomega.genPositionOfOmegaFourth,
                // Additional properties that are normally stored in the position string in the form of '+', but isn't present since it's a generated position.
                rules: { pawnDoublePush: false, castleWith: undefined },
            },
            gameruleModifications: { promotionRanks: null, moveRule: null, turnOrder: getTurnOrderOfOmega() }
     ...
     */
    const getGameRulesOfVariant = (function() {

        /**
         * Pre-calculated variant gamerules. When we request the gamerules for
         * a variant whos gamerules are not in here, we calculate them manually.
         * 
         * We avoid calculating them manually because it takes some compute to find
         * out what promotions are allowed, as we have to search through the whole position.
         */
        const runningListOfVariantGamerules = {};

        return function getGameRulesOfVariant({ Variant, UTCDate = timeutil.getCurrentUTCDate(), UTCTime = timeutil.getCurrentUTCTime() }, position) {
            if (runningListOfVariantGamerules[Variant]) return runningListOfVariantGamerules[Variant]
            
            if (!isVariantValid(Variant)) throw new Error(`Cannot get starting position of invalid variant "${Variant}"!`);
            const variantEntry = variantDictionary[Variant];

            if (!position) position = getStartingPositionOfVariant({ Variant, UTCDate, UTCTime }).position;
    ...
    function getStandarchPromotionsAllowed() { return { white: standarchPromotions, black: standarchPromotions }; }
    function getCoaIPPromotionsAllowed() { return { white: coaIPPromotions, black: coaIPPromotions }; }

    /**
     * Returns the bare-minimum gamerules a pasted game needs to function.
     * @returns {Object} The gameRules object
     */
    function getBareMinimumGameRules() {
        return {
            winConditions: getDefaultWinConditions(),
