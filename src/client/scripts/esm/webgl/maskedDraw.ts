 ...
 *
 * This is a FACTORY: {@link createMaskedDraw} builds one masker bound to a single WebGL
 * context + ProgramManager. The module default is the interactive game's masker (set at
 * runtime via {@link init}, since the gl/ProgramManager only exist once WebGL boots); the
 * variant-preview tooltip creates its own. Game-only callers use the default unchanged.
 */
...
/**
 * Must be called once after WebGL is initialized and the game's
 * ProgramManager is created, before any call to {@link execute}.
 */
function init(gl: WebGL2RenderingContext, pm: ProgramManager): MaskedDraw {
	gameInstance = createMaskedDraw(gl, pm);
	return gameInstance;
}

/** Resets the game masker's stencil index for a new frame. See {@link MaskedDraw.onFrameStart}. */
function onFrameStart(): void {
	gameInstance.onFrameStart();
}
