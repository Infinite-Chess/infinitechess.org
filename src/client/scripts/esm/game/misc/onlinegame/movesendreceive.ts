...
function handleOpponentsMove(game: Game, board: Board, mesh: Mesh | undefined, message: OpponentsMoveMessage) {
	...
	const piecemoved = boardutil.getPieceFromCoords(board.pieces, moveDraft.startCoords)!;
	const legalMoves = legalmoves.calculateAll(game, board, piecemoved);
	const endCoordsToAppendSpecial = jsutil.deepCopyObject(moveDraft.endCoords);
	legalmoves.checkIfMoveLegal(game, board, legalMoves, moveDraft.startCoords, endCoordsToAppendSpecial, onlinegame.getOpponentColor()); // Passes on any special moves flags to the endCoords
