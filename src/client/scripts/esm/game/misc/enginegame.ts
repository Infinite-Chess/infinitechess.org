...
/** If {@link move_gen_debug} is true, this contains the list of moves generated by the engine for the current position, in the form "x,y>x,y". */
let moves_generated: string[] = [];
/** Stores the legal moves generated by the engine for each move index in the game history. */
let moveHistoryLegalMoves: Map<number, string[]> = new Map();
...
function onMovePlayed(): void {
	...
	// Clear the legal moves cache for all future positions since they're no longer valid
	const currentMoveIndex = gamefile.boardsim.state.local.moveIndex;
	const movesToDelete = [];
	for (const [moveIndex] of moveHistoryLegalMoves) {
		if (moveIndex > currentMoveIndex) {
			movesToDelete.push(moveIndex);
		}
	}
	for (const moveIndex of movesToDelete) {
		moveHistoryLegalMoves.delete(moveIndex);
	}
...
function updateLegalMovesDisplay(): void {
	...
	const gamefile = gameslot.getGamefile()!;
	const currentMoveIndex = gamefile.boardsim.state.local.moveIndex;
	const storedMoves = moveHistoryLegalMoves.get(currentMoveIndex);
	moves_generated = storedMoves || [];
