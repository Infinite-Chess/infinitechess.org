...
/** Tracks the move index for the pending generated moves request. */
let pendingMoveRequestIndex: number | undefined;
...
function closeEngineGame(): void {
	...
	pendingMoveRequestIndex = undefined;
...
function handleEngineMessage(data: any): void {
	...
	if (data.type === 'move') {
		...
		// Store the moves at the move index we requested them for
		if (pendingMoveRequestIndex !== undefined) {
			moveHistoryLegalMoves.set(pendingMoveRequestIndex, [...data.data]);
			pendingMoveRequestIndex = undefined;
		}
...
function toggleDebug(): void {
	...
	// If turning off debug mode, clear any pending request
	if (!move_gen_debug) {
		pendingMoveRequestIndex = undefined;
...
function requestMovesForCurrentPosition(): void {
	...
	if (!moveHistoryLegalMoves.has(currentMoveIndex) && pendingMoveRequestIndex === undefined) {
		// Track which index we're requesting for
		pendingMoveRequestIndex = currentMoveIndex;
