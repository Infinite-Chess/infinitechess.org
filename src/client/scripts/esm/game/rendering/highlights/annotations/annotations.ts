...
function getRelevantAnnotes(): Annotes {
	...
	if (enabled) return annotes_linger;
		...
		const index = gameslot.getGamefile()!.board.state.local.moveIndex + 1; // Change -1 based to 0 based index
...
document.addEventListener('lingering-annotations-toggle', (e: CustomEvent) => {
	...
	const ply = gameslot.getGamefile()!.board.state.local.moveIndex + 1; // Change -1 based to 0 based index
