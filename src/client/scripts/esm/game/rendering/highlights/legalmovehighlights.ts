...
import type { Board, Game } from '../../../chess/logic/gamefile.js';
...
function generateModelsForPiecesLegalMoveHighlights(coords: Coords, legalMoves: LegalMoves, friendlyColor: Player, highlightColor: Color): { NonCaptureModel: BufferModelInstanced, CaptureModel: BufferModelInstanced } {
	...
	concatData_HighlightedMoves_Individual(instanceData_NonCapture, instanceData_Capture, legalMoves!, gamefile.board);
	// Potentially infinite data on sliding moves...
	concatData_HighlightedMoves_Sliding(instanceData_NonCapture, instanceData_Capture, coords, legalMoves!, gamefile, gamefile.board, friendlyColor);
 ...
 */
function concatData_HighlightedMoves_Individual(instanceData_NonCapture: number[], instanceData_Capture: number[], legalMoves: LegalMoves, board: Board) {
	...
	for (const coord of legalIndividuals) {
		const isPieceOnCoords = boardutil.isPieceOnCoords(board.pieces, coord);
 ...
 * @param friendlyColor - The color of friendly pieces
 */
function concatData_HighlightedMoves_Sliding(instanceData_NonCapture: number[], instanceData_Capture: number[], coords: Coords, legalMoves: LegalMoves, game: Game, board: Board, friendlyColor: Player) { // { left, right, bottom, top} The size of the box we should render within
	...
	for (const [lineKey, limits] of Object.entries(legalMoves.sliding)) { // '1,0'
		...
		concatData_HighlightedMoves_Diagonal(instanceData_NonCapture, instanceData_Capture, coords, line, intsect1Tile, intsect2Tile, limits, legalMoves.ignoreFunc, game, board, friendlyColor, legalMoves.brute);
	}
}
 ...
 */
function concatData_HighlightedMoves_Diagonal(instanceData_NonCapture: number[], instanceData_Capture: number[], coords: Coords, step: Vec2, intsect1Tile: Coords, intsect2Tile: Coords, limits: Coords, ignoreFunc: IgnoreFunction, game: Game, board: Board, friendlyColor: Player, brute?: boolean) {
	// Right moveset
	concatData_HighlightedMoves_Diagonal_Split(instanceData_NonCapture, instanceData_Capture, coords, step,    intsect1Tile, intsect2Tile, limits[1], 		    ignoreFunc, game, board, friendlyColor, brute);
    
	// Left moveset
	const negStep: Coords = [step[0] * -1, step[1] * -1];
	concatData_HighlightedMoves_Diagonal_Split(instanceData_NonCapture, instanceData_Capture, coords, negStep, intsect1Tile, intsect2Tile, Math.abs(limits[0]), ignoreFunc, game, board, friendlyColor, brute);
}
 ...
 * @param brute - If true, each move will be simulated as to whether it results in check, and if so, not added to the mesh data.
 */
function concatData_HighlightedMoves_Diagonal_Split(instanceData_NonCapture: number[], instanceData_Capture: number[], coords: Coords, step: Vec2, intsect1Tile: Coords, intsect2Tile: Coords, limit: number, ignoreFunc: IgnoreFunction, game: Game, board: Board, friendlyColor: Player, brute?: boolean) {
	...
	addDataDiagonalVariant(instanceData_NonCapture, instanceData_Capture, firstInstancePositionOffset, step, iterationCount, startCoords, coords, ignoreFunc, game, board, friendlyColor, brute);
}
 ...
 * @param brute - If true, each move will be simulated as to whether it results in check, and if so, not added to the mesh data.
 */
function addDataDiagonalVariant(instanceData_NonCapture: number[], instanceData_Capture: number[], firstInstancePositionOffset: Coords, step: Vec2, iterateCount: number, startCoords: Coords, pieceCoords: Coords, ignoreFunc: IgnoreFunction, game: Game, board: Board, friendlyColor: Player, brute?: boolean) {
	for (let i = 0; i < iterateCount; i++) { 
		...
		legal: if (ignoreFunc(pieceCoords, thisCoord)) { // Ignore function PASSED. This move is LEGAL
			...
			if (brute) {
				...
				if (checkresolver.getSimulatedCheck(game, board, moveDraft, friendlyColor).check) break legal;
			}

			// Should we add instance data to the capturing or non-capturing model?
			const isPieceOnCoords = boardutil.isPieceOnCoords(board.pieces, thisCoord);
