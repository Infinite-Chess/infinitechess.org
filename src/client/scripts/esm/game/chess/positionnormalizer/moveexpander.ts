// src/client/scripts/esm/game/chess/positionnormalizer/moveexpander.ts
...
function expandMove(
	...
	const originalPiece = pieceTransformations.find((pt) =>
	...
	const capturedTransformedPiece = pieceTransformations.find((pt) =>
		coordutil.areCoordsEqual(pt.transformedCoords as Coords, endCoordsBigInt),
	);
	if (capturedTransformedPiece) {
		// EASY! Return the captured piece's original coords
		return {
	...
	const vector: Vec2 = vectors.absVector(
		vectors.normalizeVector(coordutil.subtractCoords(endCoordsBigInt, startCoordsBigInt)),
	);
	...
	const movementLine: LineCoefficients = vectors.getLineGeneralFormFromCoordsAndVec(
		originalStartCoords,
		vector,
	);
	...
	function determineIfMovedPieceInterestedInAxis(axis: '1,0' | '0,1'): void {
		...
		for (const axisGroup of axisOrder) {
			if (
				compressedEndCoordsAxisValue + HALF_ARBITRARY_DISTANCE >=
					axisGroup.transformedRange![0] &&
				compressedEndCoordsAxisValue - HALF_ARBITRARY_DISTANCE <=
				...
				const offsetFromGroupStart =
					compressedEndCoordsAxisValue - axisGroup.transformedRange![0];
				...
				originalEndCoords = trueEndCoordsDeterminer(
					movementLine,
					axis,
					actualEndCoordsAxisValue,
				);
				break;
			}
		}
		if (!originalEndCoords) {
			...
			if (
				...
				const distToRightMostGroup =
					...
					axisOrder[axisOrder.length - 1]!.transformedRange![1];
				const actualEndCoordsAxisValue =
					axisOrder[axisOrder.length - 1]!.range[1] + distToRightMostGroup;
				// The ACTUAL coordinates they moved to!
				originalEndCoords = trueEndCoordsDeterminer(
					movementLine,
					axis,
	...
	if (!originalEndCoords)
		throw Error('Unable to determine the original end coordinates of the moved piece! ');
 ...
 */
function trueEndCoordsDeterminer(
	movementLine: LineCoefficients,
	axisOfInterest: '1,0' | '0,1',
	targetAxisValue: bigint,
): Coords {
	// console.log("Determining true end coords for axis:", axisOfGroupOfInterest, " with target axis value:", targetAxisValue);

	const axisPerpendicularVec: Vec2 = vectors.getPerpendicularVector(
		vectors.getVec2FromKey(axisOfInterest),
	);
	...
	if (axisOfInterest === '1,0') {
		...
		intersectionLine = vectors.getLineGeneralFormFromCoordsAndVec(
			[targetAxisValue, 0n],
			axisPerpendicularVec,
		);
	...
	const intersectionPoint: BDCoords | undefined = geometry.calcIntersectionPointOfLines(
		...movementLine,
		...intersectionLine,
	);
