 ...
 * @typedef {import("../../../../../../../chess/logic/organizedpieces.js").OrganizedPieces} OrganizedPieces
...
export function getCoordsOfAllPieces(game) {
	...
	try {
		...
		if (boardutil && typeof boardutil.getCoordsOfAllPieces === 'function') {
			try {
				const result = boardutil.getCoordsOfAllPieces(game.boardsim.pieces || game);
		...
		const pieces = game.boardsim.pieces || game;
...
export function getPieceCountOfGame(game) {
	return boardutil.getPieceCountOfGame(game.boardsim.pieces, "major_or_minor");
}
...
export function getLegalMoves(lf, player) {
	const legalMoves = [];
	const allPieceCoords = boardutil.getCoordsOfAllPieces(lf.boardsim.pieces);
	...
	for (const coords of allPieceCoords) {
		...
		const legalMovesResult = legalmoves.calculateAll(lf, piece);
