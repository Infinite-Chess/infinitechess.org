...
function createCheckState(move: Move | Edit, current: inCheck, future: inCheck, gamefile: gamefile) {
...
function createAttackersState(move: Move | Edit, current: attackers, future: attackers, gamefile: gamefile) {
...
function createSpecialRightsState(move: Move | Edit, coordsKey: CoordsKey, current?: true, future?: true) {
	if (current === future) return; // If the current and future values are identical, we can skip queueing this state.
...
/**
 * Applies all the StateChanges of a Move | Edit, in order, to the gamefile,
