 ...
 */
function removeCheckInvalidMoves(game: Game, board: Board, pieceSelected: Piece, moves: LegalMoves): void {
	...
	removeCheckInvalidMoves_Sliding(game, board, moves, pieceSelected, color);

	// 2. Individual moves. We can iterate through these and use detectCheck() to test them.
	removeCheckInvalidMoves_Individual(game, board, moves.individual, pieceSelected, color);
 ...
 */
function removeCheckInvalidMoves_Individual(game: Game, board: Board, individualMoves: CoordsSpecial[], piece: Piece, color: Player): void { // [ [x,y], [x,y] ]
	...
	for (let i = individualMoves.length - 1; i >= 0; i--) { // Iterate backwards so we don't run into issues as we delete indices while iterating
		...
		if (isMoveCheckInvalid(game, board, piece, thisMove, color)) individualMoves.splice(i, 1); // Remove the move
	}
}
 ...
 * @param color - The color of the player the piece belongs to.
 */
function removeCheckInvalidMoves_Sliding(game: Game, board: Board, moves: LegalMoves, piece: Piece, color: Player): void {
	if (!moves.sliding) return; // No sliding moves to remove

	/** List of coordinates of all our royal jumping pieces */
	const royalCoords: Coords[] = boardutil.getJumpingRoyalCoordsOfColor(board.pieces, color);
	...
	if (addressExistingChecks(game, board, moves, royalsInCheck, piece.coords, color)) return;
	/**
	 ...
	 */
	else removeSlidingMovesThatOpenDiscovered(game, board, moves, piece, color);
}
 ...
 */
function addressExistingChecks(game: Game, board: Board, legalMoves: LegalMoves, royalCoords: Coords[], selectedPieceCoords: Coords, color: Player): boolean {
	if (royalCoords.length === 0) return false; // Exit if nothing in check
	if (!checkdetection.isPlayerInCheck(board, color)) return false; // Our OPPONENT is in check, not us! Them being in check doesn't restrict our movement!

	const attackerCount = board.state.local.attackers.length;
	...
	const attacker = board.state.local.attackers[0]!; // { coords, slidingCheck }
	...
	const capturingImpossible = attackerCount > 1 && !board.colinearsPresent; // With a double check, it's impossible to capture both pieces at once, forced to dodge with the king.
	// Check if the piece has the ability to capture
	if (!capturingImpossible && legalmoves.checkIfMoveLegal(game, board, legalMoves, selectedPieceCoords, attacker.coords, color, { ignoreIndividualMoves: true })) {
	...
	if (attacker.slidingCheck) appendBlockingMoves(game, board, royalCoords[0]!, attacker.coords, legalMoves, selectedPieceCoords, color); // Has a chance to delete all sliding moves except one, adding the `brute` flag.
	else appendPathBlockingMoves(game, board, attacker.path!, legalMoves, selectedPieceCoords, color);
 ...
 * @param color - The color of the player the piece belongs to.
 */
function removeSlidingMovesThatOpenDiscovered(game: Game, board: Board, moves: LegalMoves, pieceSelected: Piece, color: Player): void {
	if (checkdetection.isPlayerInCheck(board, color)) throw Error('We should not be in check when calling removeSlidingMovesThatOpenDiscovered!'); // Safety net
	if (!moves.sliding) return; // No sliding moves to remove
	...
	boardchanges.runChanges({game, board}, deleteChange, boardchanges.changeFuncs, true);

	const checkResults = checkdetection.detectCheck(game, board, color, true); // { check: boolean, royalsInCheck: Coords[], attackers: Attacker[] }
	...
	outer: if (checkResults.check) {
		...
		for (const checkedRoyalCoords of checkResults.royalsInCheck) {
			for (const attacker of checkResults.attackers!) {
				...
				if (!attacker.slidingCheck) { // This attacker is giving a check via a special individual move with a `path` (such as the Rose piece).
					...
					appendPathBlockingMoves(game, board, attacker.path!, moves, pieceSelected.coords, color);
					// Also append the capturing move if it's legal
					if (legalmoves.checkIfMoveLegal(game, board, moves, pieceSelected.coords, attacker.coords, color, { ignoreIndividualMoves: true })) {
		...
		else if (board.colinearsPresent) moves.brute = true;
	}

	boardchanges.runChanges({game, board}, deleteChange, boardchanges.changeFuncs, false); // Add the piece back
 ...
 * @param color - The color of friendlies
 */
function appendBlockingMoves(game: Game, board: Board, square1: Coords, square2: Coords, moves: LegalMoves, coords: Coords, color: Player): void { // coords is of the selected piece
	...
	for (const lineKey in moves.sliding) { // 'dx,dy'
		...
		if (blockPoint === undefined && board.colinearsPresent && math.areLinesInGeneralFormEqual(line1GeneralForm, line2GeneralForm)) {
		...
		if (board.colinearsPresent && moves.individual.some((move: CoordsSpecial) => move[0] === blockPoint[0] && move[1] === blockPoint[1])) continue;

		// Can our piece legally move there?
		if (legalmoves.checkIfMoveLegal(game, board, moves, coords, blockPoint, color, { ignoreIndividualMoves: true })) moves.individual.push(blockPoint); // Can block!
	}
}
 ...
 * @param color - The color of friendlies
 */
function appendPathBlockingMoves(game: Game, board: Board, path: path, legalMoves: LegalMoves, selectedPieceCoords: Coords, color: Player): void {
	...
	for (let i = 1; i < path.length - 1; i++) { // Iterate through all path points, EXCLUDING start and end.
		...
		if (legalmoves.checkIfMoveLegal(game, board, legalMoves, selectedPieceCoords, blockPoint, color, { ignoreIndividualMoves: true })) legalMoves.individual.push(coordutil.copyCoords(blockPoint)); // Can block!
	}
}
 ...
 */
function isMoveCheckInvalid(game: Game, board: Board, piece: Piece, destCoords: CoordsSpecial, color: Player) { // pieceSelected: { type, index, coords }
	...
	return getSimulatedCheck(game, board, moveDraft, color).check;
}
 ...
 */
function getSimulatedCheck(game: Game, board: Board, moveDraft: MoveDraft, colorToTestInCheck: Player): ReturnType<typeof checkdetection.detectCheck> {
	return movepiece.simulateMoveWrapper(
		game, board,
		moveDraft,
		() => checkdetection.detectCheck(game, board, colorToTestInCheck),
	);	
}
