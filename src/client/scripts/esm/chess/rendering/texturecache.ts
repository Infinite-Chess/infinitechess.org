 ...
 */
async function initTexturesForGame(gl: WebGL2RenderingContext, board: Board): Promise<void> {
	...
	const types = board.existingTypes.filter((t: number) => !(typeutil.getRawType(t) in typeutil.SVGLESS_TYPES) );
