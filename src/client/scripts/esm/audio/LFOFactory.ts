...
/**
 * Zoom level of the simplex noise that determines the slowness and smoothness of it.
 * Higher = slower changes, lower = faster changes.
 */
const NOISE_ZOOM = 40000;
...
function createPerlinLFO(context: AudioContext, rate: number): AudioBufferSourceNode {
	if (!perlinNoiseBuffer) { // Create the perlin noise buffer only once
		...
		for (let i = 0; i < sampleCount; i++) {
			data[i] = SimplexNoise.noise(i / NOISE_ZOOM, 0);
		}
	}
