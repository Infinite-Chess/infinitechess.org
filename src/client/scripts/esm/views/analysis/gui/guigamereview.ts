...
function init(): void {
	...
	// Registered after guimoveslist's own listener (module scope), so the move tree the
	// snapshot reads has already been rebuilt by the time this runs.
...
function snapshotPristineMainline(): void {
	pristineMainline = currentMainline();
}
...
/**
 * Closes the outgoing game's review UI, so nothing of it can outlive the game it describes.
 * gamereview discards its own state on the same event.
 */
...
function onGameReviewClicked(): void {
	if (currentMainline() === pristineMainline) {
		...
		if (!proceed) return;
		// Reloading in place (rather than refreshing the page) keeps the engine warm, and is the
		// only option on the bare page, whose position exists nowhere but memory.
		void analysisloader.reloadPristine().then(openReview);
	}
}
