// src/client/scripts/esm/views/checkmatepractice/practicegame.ts

/**
 * This script handles checkmate practice logic: generating a checkmate's starting
 * position, running the local game against the practice bot, undo/restart, and
...
/**
 * Half-width of the world border every practice game runs inside — the engine is never
 * handed an unbounded board. ~11% of Number.MAX_SAFE_INTEGER, which freezes the engine
 * if you move to the border.
 */
const WORLD_BORDER_DIST = BigInt(1e15);
...
/** Whether the player is allowed to undo a move in the current position. */
function isUndoingLegal(): boolean {
	return undoingIsLegal;
}

/**
 * Starts a checkmate practice game.
 * @returns Whether the game actually started (refused while another load is in flight).
 */
function startCheckmatePractice(checkmateSelectedID: string): boolean {
	// Don't stomp an in-flight load — unloading its gamefile would crash its graphical half.
	if (gamesession.isLoading()) {
		toast.showPleaseWaitForTask();
		return false;
	}
	...
	const variantOptions: VariantOptions = {
		...
		gameRules: {
			...variantrules.getBareMinimumGameRules(),
			worldBorder: {
				left: -WORLD_BORDER_DIST,
				right: WORLD_BORDER_DIST,
				bottom: -WORLD_BORDER_DIST,
				top: WORLD_BORDER_DIST,
			},
		},
	};
	const engine = {
	...
	startEngineGame({ variantOptions, youAreColor: p.WHITE, engine });
	return true;
}

/** Loads a local, untimed game against the practice bot. */
function startEngineGame(options: {
	variantOptions: VariantOptions;
	youAreColor: Player;
	engine: EngineAndConfig;
}): void {
	gamesession.setSessionGame({ type: 'practice', role: options.youAreColor });
