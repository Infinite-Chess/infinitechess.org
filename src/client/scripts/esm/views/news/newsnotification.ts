// src/client/scripts/esm/views/news/newsnotification.ts

/**
 * The unread-news badge on the header's News link: fetches the unread count and paints a
 * red circle next to the link.
 *
 * DORMANT — nothing imports this. It is the pre-redesign badge, kept for the logic worth
 * repurposing: the logged-out early exit, the "9+" cap, and the `login` / `news-marked-read`
 * events it refreshes on. Revisit when the redesigned news page lands, at which point:
 *  - the inline `cssText` block below must go — styling belongs in a stylesheet, not a script,
 *  - and the badge element itself belongs in the header's `.njk` template, leaving this script
 *    to toggle and fill it rather than build it.
 */
