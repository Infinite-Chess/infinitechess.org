...
function renderNukeSites<T extends Gamesim & AtomicData>(gamefile: T): false {
	...
	if (
		!selection.isAPieceSelected() ||
		!hover ||
		!boardutil.isPieceOnCoords(gamefile.boardsim.pieces, hover)
	)
		return false;
	if (
		typeutil.getColorFromType(selection.getPieceSelected()!.type) ===
		typeutil.getColorFromType(
			boardutil.getPieceFromCoords(gamefile.boardsim.pieces, hover)!.type,
		)
	)
		return false;
	squarerendering
		.genModel(
			gamefile.atomic.nukeRange.map((n) => coordutil.addCoords(n, hover)),
			[1, 0, 0, 0.4],
		)
		.render();
	return false;
