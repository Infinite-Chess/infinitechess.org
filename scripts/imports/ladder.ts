 ...
 *   sccs                File-level cycles. One inside a single directory is ladder-legal, still a cycle.
...
for (const f of allFiles) {
	...
	// Classify type-only separately with the AST (preProcessFile does not expose it).
	const sf = ts.createSourceFile(f, text, ts.ScriptTarget.Latest, true);
	const markRuntime = (spec: string): void => {
		const r = resolveSpec(f, spec);
		if (r && r !== f) runtimeEdge.add(`${f}->${r}`);
	};
	for (const stmt of sf.statements) {
		if (ts.isImportDeclaration(stmt) && ts.isStringLiteral(stmt.moduleSpecifier)) {
			...
			if (!typeOnly) markRuntime(stmt.moduleSpecifier.text);
	...
	const visitDynamic = (n: ts.Node): void => {
		if (ts.isCallExpression(n) && n.expression.kind === ts.SyntaxKind.ImportKeyword) {
			...
			if (a && ts.isStringLiteral(a) && a.text.startsWith('.')) markRuntime(a.text);
		}
